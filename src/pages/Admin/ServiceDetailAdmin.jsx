...
const ServiceDetailAdmin = () => {
  ...
  useEffect(() => {
    const getService = async () => {
      try {
        const response = await axios.get(`https://easyservice-backend-iv29.onrender.com/api/services/${id}`, {
          headers: {
  ...
  return (
    <div className="container mx-auto px-4">
      <a href="/admin/services" className="text-gray-700 px-2 py-1 bg-gray-200 hover:bg-gray-300 rounded">Retour</a>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="lg:w-1/3">
            <div className="bg-orange-50 shadow-md p-6 rounded-lg border border-orange-100">
              <p className="mb-3">
                <strong>TARIF :</strong> {service.tarif ? `${service.tarif} FCFA` : "Non spécifié"}
              </p>
              <p className="mb-3">
                <strong>DUREE :</strong> {service.duree ? `${service.duree} ${service.uniteDuree || ""}` : "Non spécifiée"}
              </p>
              <p className="mb-3">
                <strong>DATE INTERVENTION :</strong> {formatDate(service.dateIntervention)}
              </p>
              <p className="text-sm text-gray-600 mt-4">
                Ajouté le {service.createDate ? new Date(service.createDate).toLocaleDateString('fr-FR') : "date inconnue"} par l&apos;admin{" "}
                <strong className="text-orange-500 uppercase">{service.admin?.prenom} {service.admin?.nom}</strong>
              </p>
            </div>
          </div>
        </div>
