...
export default function DetailsDemandeClient() {
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="text-center text-red-500">
          ...
          <Link to="/client/demandes" className="text-orange-500 hover:underline flex items-center justify-center">
            <FiArrowLeft className="mr-2" /> Retour à la liste des demandes
          </Link>
        </div>
      </div>
    );
  ...
  return (
    <div className="container mx-auto sm:px-4 sm:py-8">
      <Link to="/client/demandes" className="inline-flex items-center mb-4 text-orange-500 hover:underline">
        <FiArrowLeft className="mr-2" /> Retour aux demandes
      </Link>
          ...
          <div className="flex flex-col flex-wrap h-fit">
            ...
            <div className="space-y-3">
              <p>
                ...
                <span className="capitalize">{demande.admin?.prenom ? demande.admin.prenom : "Non assigné"} {demande.admin?.nom}</span>
              </p>
              <p>
                <span className="font-semibold">Client :</span>{" "}
                {demande.client?.prenom} {demande.client?.nom} (Vous)
              </p>
              <p>
        ...
        {demande.statut === "terminee" && (
          <div className="">
            <div className="flex flex-col-reverse justify-end items-end sm:flex-row gap-2 fixed bottom-5 sm:right-8 right-5">
              ...
              >
                <FiDownload className="mr-2" /> Facture
              </button>
              <button
                className={`${
                  showFeedbackForm ? "bg-gray-500" : "bg-orange-500"
                } text-white px-4 py-2 rounded hover:bg-orange-600 flex items-center`}
                onClick={() => setShowFeedbackForm(!showFeedbackForm)}
              >
                {showFeedbackForm ? (
                  <>
                    <FiX className="mr-2" /> Masquer
                  </>
                ) : (
                  <>
            ...
            <form onSubmit={handleFeedbackSubmit} className="space-y-4">
              ...
              <div className="flex justify-end gap-2">
                ...
                <button
                  ...
                  className="px-4 py-2 text-white bg-orange-500 rounded-lg hover:bg-orange-600 disabled:opacity-50 flex items-center"
                >
                  {isSubmitting ? (
                    <>En cours...</>
                  ) : (
                    <>
                      <FiCheck className="mr-2" /> Envoyer
                    </>
                  )}
                </button>
              </div>
